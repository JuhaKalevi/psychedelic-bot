...
async def generate_summary_from_transcription(message, model='gpt-4'):
  response = await openai_chat_completion([{'role': 'user', 'content': "Summarize in appropriate detail, adjusting the summary length according to the transcription's legnth, the YouTube-video transcription below. 
                                          Also make guess how many different characters' speech is included in the transcription. Also analyze the style of this video (comedy, drama, instructional, educational, etc.). IGNORE all advertisement(s), sponsorship(s), discount(s), promotions(s), all War Thunder/Athletic Green etc. talk completely. Transcription:" + message}], model)
  return response
