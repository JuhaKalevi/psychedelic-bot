...
async def context_manager(event):
  ...
  if 'event' in event and event['event'] == 'posted' and event['data']['sender_name'] != environ['MATTERMOST_BOTNAME']:
    ...
    if '@code-analysis' in post['message']:
      ...
      messages.append({'role':'system', 'content':"Note that before each user message you have a system message which contains the user's name and date & time of the message. You can determine current time from the latest timestamp from these messages, instead of saying that it is not possible. If usernames or names of conversation participants are requested, use this information."})
    ...
    for post_id in context['order']:
      ...
      if 'from_bot' in context['posts'][post_id]['props']:
        ...
        role = 'user'
        messages.append({'role': 'system', 'content': f'The following message is from user {post_username}, timestamp '+str(datetime.fromtimestamp(post['create_at']/1000).strftime("%Y-%m-%d %H:%M"))})
