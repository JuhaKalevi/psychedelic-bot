  ...
  if 'event' in event and event['event'] == 'posted' and event['data']['sender_name'] != environ['MATTERMOST_BOTNAME']:
    ...
    for post_id in context['order']:
      ...
      if 'from_bot' in context['posts'][post_id]['props']:
        ...
        role = 'user'
    messages.append({'role': role, 'content': f'The following message is from user named {post_username}, timestamp '+str(datetime.fromtimestamp(context['posts'][post_id]['create_at']/1000).strftime("%Y-%m-%d %H:%M"))+"\n"+context['posts'][post_id]['message']})
    try:
