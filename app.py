...
async def context_manager(event):
  ...
  if new_post['root_id'] == '':
    ...
    elif is_configured_for_replies_without_tagging(mm.channels.get_channel(new_post['channel_id'])):
      thread_id = ''
      context = mm.posts.get_posts_for_channel(new_post['channel_id'])
    if new_post['message'].lower().startswith("2x"):
      ...
      openai_response_content = await upscale_image_4x(file_ids, new_post)
