        ...
        if r['images']:
          for img_b64 in r['images']:
            ...
            with io.BytesIO() as output:
              ...
              output.seek(0)
              uploaded_file_id = await self.client.upload_file(self.reply_to, {'files':(f'/tmp/image_{total_images_saved}_{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}.png', output)})
