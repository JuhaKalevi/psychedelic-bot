        ...
        if r['images']:
          for img_b64 in r['images']:
            ...
            with io.BytesIO() as output:
              ...
              output.seek(0)
              uploaded_file_id = await self.client.upload_file(self.thread['channel_id'], {'files':(f'/tmp/image_{total_images_saved}_{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}.png', output)})
            await self.client.create_or_update_post({'channel_id':self.thread['channel_id'], 'file_ids':[uploaded_file_id], 'root_id':''})
