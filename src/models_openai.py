  ...
  f_choose = [
    {
      ...
      'description': "This function is used to select which of the actual functions should be called. If the user mentions a function by name but the message doesn't really seem to be a request for that function, don't call it then!",
      'parameters': {
        'type': 'object',
        'properties': {
          'function_name': {
...
async def chat_completion(msgs, model='gpt-4-1106-preview', max_tokens=None):
  kwargs = {'messages':msgs, 'model':model, 'stream':True, 'temperature':2, 'top_p':0.25}
  if max_tokens:
