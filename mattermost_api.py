...
async def _channel_display_name_from_post(post, bot):
  return bot.channels.get_channel(post['channel_id'])['display_name']

async def channel_context(post, bot):
...
async def create_post(options, bot):
  try:
