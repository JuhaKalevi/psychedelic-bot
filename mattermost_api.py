...
def channel_context(post, bot):
  print(f"{_channel_display_name_from_post(post)}: len(channel_context): {len(channel_context)}")
  return bot.posts.get_posts_for_channel(post['channel_id'])

def channel_from_post(post, bot):
  print(f"{_channel_display_name_from_post(post)}: channel_from_post")
...
def thread_context(post, bot):
  print(f"{_channel_display_name_from_post(post)}: thread_context'")
